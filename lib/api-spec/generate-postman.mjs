...
try {
  ...
  // When the generator is run from the workspace root (e.g. via the api-server
  // build step), js-yaml may not resolve from lib/api-spec/node_modules.
  // Resolve it from artifacts/api-server which declares js-yaml as a direct dep.
  const apiServerRequire = createRequire(
    resolve(__dirname, "../../artifacts/api-server/package.json")
  );
  jsYaml = apiServerRequire("js-yaml");
}
