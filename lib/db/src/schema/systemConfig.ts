...
export const SYSTEM_CONFIG_KEYS = {
  ...
  PASSWORD_RESET_DELIVERY_RETENTION_DAYS: "password_reset_delivery_retention_days",
  ...
  UPIGATEWAY_DAILY_LIMIT: "upigateway_daily_limit",
  UPIGATEWAY_MERCHANT_ACCESS: "upigateway_merchant_access",
  UPIGATEWAY_CAP_WARNING_THRESHOLD: "upigateway_cap_warning_threshold",
...
export const SYSTEM_CONFIG_DEFAULTS = {
  ...
  [SYSTEM_CONFIG_KEYS.PASSWORD_RESET_DELIVERY_RETENTION_DAYS]: "30",
  ...
  [SYSTEM_CONFIG_KEYS.UPIGATEWAY_DAILY_LIMIT]: "1000000",
  [SYSTEM_CONFIG_KEYS.UPIGATEWAY_MERCHANT_ACCESS]: "false",
  [SYSTEM_CONFIG_KEYS.UPIGATEWAY_CAP_WARNING_THRESHOLD]: "80",
...
/**
 * Keys intentionally excluded from SYSTEM_CONFIG_DEFAULTS.
 *
 * Every value in SYSTEM_CONFIG_KEYS must appear in either SYSTEM_CONFIG_DEFAULTS
 * or here — the coverage test (`lib/db/src/schema/systemConfig.coverage.test.ts`)
 * enforces this invariant at dev/CI time.
 *
 * Two valid reasons to list a key here:
 *   "credential"     — must be configured per-environment; an empty default is unsafe
 *   "runtime-state"  — written by the system/scheduler at runtime, never seeded
 *
 * If you add a key to SYSTEM_CONFIG_KEYS and omit a default, you MUST add it here
 * with a comment explaining why, or the coverage test will fail.
 */
export const SYSTEM_CONFIG_NO_DEFAULT_KEYS: ReadonlySet<string> = new Set([
  // ── runtime-state: written by scheduler/system, never seeded ──────────────
  ...
  SYSTEM_CONFIG_KEYS.RAZORPAY_X_FAILURE_REASON,
  ...
  SYSTEM_CONFIG_KEYS.CASHFREE_BASE_URL,             // environment-specific; set by admin config
  ...
  SYSTEM_CONFIG_KEYS.CASHFREE_PAYOUT_BASE_URL,      // environment-specific; set by admin config
