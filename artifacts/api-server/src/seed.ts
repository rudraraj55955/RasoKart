...
export async function seed() {
  ...
  // Runs on every start: upserts all ALL_PERMISSION_KEYS into the permissions
  // table and fills any missing role_permissions rows.  The upsert is fully
  // idempotent; new keys added after the initial migration are covered without
  // a re-migration.  The role_permissions insert uses onConflictDoNothing so
  // admin-customised role templates are never overwritten.
  if (iamMigRow) {
    // ── 1. Sync permissions catalog (always — idempotent upsert) ────────────
    for (const key of ALL_PERMISSION_KEYS) {
      const category = key.split("_")[0] ?? "unknown";
      const isSuperAdminOnly = SUPER_ADMIN_ONLY_PERMISSIONS.has(key);
      await db
  ...
  // ── Razorpay capability audit products seed ───────────────────────────────
  // Idempotent — uses productKey as conflict target.
  // Only updates fields that drive the capability matrix UI;
  // never overwrites admin-entered implNotes or docsUrl.
  try {
    const RAZORPAY_CAPABILITY_PRODUCTS: Array<typeof providerProductsTable.$inferInsert> = [
      {
        providerKey: "razorpay",
        productKey: "razorpay_payin",
        publicName: "Payment Collection (Payin)",
        description: "Accept card, UPI, netbanking, wallet, EMI, and Pay Later payments from customers.",
        isEnabled: true,
        status: "live",
        sortOrder: 1,
        capabilityStatus: "ALREADY_LIVE_IN_RASOKART",
        officialApiAvailable: true,
        officialSdkAvailable: true,
        testModeStatus: "configured",
        liveModeStatus: "configured",
        webhookSupport: true,
        webhookEvents: ["payment.captured", "payment.failed", "order.paid"],
        approvalRequired: false,
        merchantAccess: true,
        customerFacingModule: true,
        implNotes: "Fully implemented in RasoKart. Orders, HMAC signature verification, and webhook processing all working. Razorpay Checkout JS SDK integrated.",
        docsUrl: "https://razorpay.com/docs/payment-gateway/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_refunds",
        publicName: "Refunds",
        description: "Initiate full or partial refunds for captured Razorpay payments.",
        isEnabled: true,
        status: "live",
        sortOrder: 2,
        capabilityStatus: "ALREADY_IMPLEMENTED_NEEDS_TESTING",
        officialApiAvailable: true,
        officialSdkAvailable: true,
        testModeStatus: "configured",
        liveModeStatus: "configured",
        webhookSupport: true,
        webhookEvents: ["refund.created", "refund.processed", "refund.failed", "refund.speed_changed"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Admin-initiated refund management implemented. Refunds stored in razorpay_refunds table. Normal and Optimum speed supported. Needs live-credential end-to-end test.",
        docsUrl: "https://razorpay.com/docs/api/refunds/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_orders",
        publicName: "Orders API",
        description: "Create and manage Razorpay orders used to track payment sessions end-to-end.",
        isEnabled: true,
        status: "live",
        sortOrder: 3,
        capabilityStatus: "ALREADY_LIVE_IN_RASOKART",
        ...
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Orders are created server-side before each Checkout session and tracked in razorpay_payment_orders table.",
        docsUrl: "https://razorpay.com/docs/api/orders/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_payment_links",
        publicName: "Payment Links",
        description: "Create shareable payment links — no custom checkout required.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 4,
        capabilityStatus: "NEW_INTEGRATION_REQUIRED",
        officialApiAvailable: true,
        officialSdkAvailable: false,
        testModeStatus: "not_configured",
        liveModeStatus: "not_configured",
        webhookSupport: true,
        webhookEvents: ["payment_link.paid", "payment_link.expired", "payment_link.cancelled"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Razorpay Payment Links API is fully public. Not yet built into RasoKart — planned for merchant self-serve portal.",
        docsUrl: "https://razorpay.com/docs/payment-links/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_settlements",
        publicName: "Settlements",
        description: "Fetch settlement details, UTR, and bank transfer records from Razorpay.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 5,
        capabilityStatus: "PARTIALLY_IMPLEMENTED",
        officialApiAvailable: true,
        officialSdkAvailable: false,
        testModeStatus: "not_configured",
        liveModeStatus: "verification_required",
        webhookSupport: true,
        webhookEvents: ["settlement.processed"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Admin settlement overview panel exists (manual cache in system_config). Razorpay Settlements API (GET /v1/settlements) not yet auto-synced. No settlement.processed webhook handler.",
        docsUrl: "https://razorpay.com/docs/api/settlements/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_subscriptions",
        publicName: "Subscriptions",
        description: "Recurring billing — automated charge on a fixed schedule with plan management.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 6,
        capabilityStatus: "RAZORPAY_APPROVAL_REQUIRED",
        officialApiAvailable: true,
        officialSdkAvailable: true,
        testModeStatus: "not_configured",
        liveModeStatus: "approval_required",
        webhookSupport: true,
        webhookEvents: ["subscription.activated", "subscription.charged", "subscription.completed", "subscription.updated", "subscription.cancelled", "subscription.paused"],
        approvalRequired: true,
        approvalReason: "Razorpay Subscriptions requires account activation via Razorpay Support. Submit a request at razorpay.com/support.",
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "API available post-approval. Not yet implemented in RasoKart.",
        docsUrl: "https://razorpay.com/docs/subscriptions/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_qr_codes",
        publicName: "QR Codes (Smart Collect)",
        description: "Generate static or dynamic QR codes for UPI payments via Smart Collect.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 7,
        capabilityStatus: "NEW_INTEGRATION_REQUIRED",
        ...
        liveModeStatus: "not_configured",
        webhookSupport: true,
        webhookEvents: ["qr_code.credited", "qr_code.closed"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Smart Collect QR Codes API available. Not yet implemented in RasoKart (RasoKart uses EKQR/Cashfree for QR). Planned integration.",
        docsUrl: "https://razorpay.com/docs/payments/qr-codes/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_virtual_accounts",
        publicName: "Virtual Accounts (Smart Collect)",
        description: "Create bank account numbers for receiving bulk transfers — reconciled automatically.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 8,
        capabilityStatus: "NEW_INTEGRATION_REQUIRED",
        ...
        liveModeStatus: "verification_required",
        webhookSupport: true,
        webhookEvents: ["virtual_account.credited", "virtual_account.closed"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Smart Collect Virtual Accounts require a banking partner agreement (ICICI/Yes Bank). Additional verification needed. Not yet implemented in RasoKart.",
        docsUrl: "https://razorpay.com/docs/payments/smart-collect/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_customers",
        publicName: "Customers API",
        description: "Create and manage customer records, attach saved payment methods.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 9,
        capabilityStatus: "NEW_INTEGRATION_REQUIRED",
        ...
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Customer management API is fully public. Not yet implemented in RasoKart merchant portal.",
        docsUrl: "https://razorpay.com/docs/api/customers/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_invoices",
        publicName: "Invoices",
        description: "Send invoices with payment links to customers — limited programmatic API.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 10,
        capabilityStatus: "DASHBOARD_ONLY",
        officialApiAvailable: true,
        officialSdkAvailable: false,
        testModeStatus: "not_configured",
        liveModeStatus: "not_configured",
        webhookSupport: true,
        webhookEvents: ["invoice.paid", "invoice.expired"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Invoice API exists but primary management is via Razorpay Dashboard. Not yet integrated into RasoKart — use DASHBOARD_ONLY until merchant-facing invoice flow is built.",
        docsUrl: "https://razorpay.com/docs/invoices/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_payment_pages",
        publicName: "Payment Pages",
        description: "No-code hosted payment pages built via Razorpay Dashboard.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 11,
        capabilityStatus: "DASHBOARD_ONLY",
        ...
        officialSdkAvailable: false,
        testModeStatus: "dashboard_only",
        liveModeStatus: "dashboard_only",
        webhookSupport: false,
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Razorpay Payment Pages are created exclusively via the Razorpay Dashboard. No public REST API exists for programmatic creation or management.",
        docsUrl: "https://razorpay.com/docs/payment-pages/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_payment_button",
        publicName: "Payment Button",
        description: "Embeddable JS widget for one-click checkout on external websites.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 12,
        capabilityStatus: "DASHBOARD_ONLY",
        ...
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Embed code generated in Dashboard only. JS SDK integration is copy-paste. No REST API for button management.",
        docsUrl: "https://razorpay.com/docs/payment-button/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_optimizer",
        publicName: "Razorpay Optimizer",
        description: "Smart routing that dynamically selects the best payment gateway per transaction.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 13,
        capabilityStatus: "DASHBOARD_ONLY",
        ...
        webhookSupport: false,
        approvalRequired: true,
        approvalReason: "Requires Razorpay Optimizer activation on the account.",
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Optimizer routing rules are configured only via the Razorpay Dashboard. No public API. RasoKart uses its own smart routing engine.",
        docsUrl: "https://razorpay.com/docs/optimizer/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_affordability",
        publicName: "Affordability Suite (EMI / BNPL)",
        description: "No-cost EMI, Pay Later, and cardless EMI options at checkout.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 14,
        capabilityStatus: "RAZORPAY_APPROVAL_REQUIRED",
        ...
        approvalRequired: true,
        approvalReason: "Requires Razorpay Affordability Suite activation. Merchant eligibility check required.",
        merchantAccess: true,
        customerFacingModule: true,
        implNotes: "EMI and BNPL presented at Checkout automatically once enabled. Requires account approval and eligible merchant category.",
        docsUrl: "https://razorpay.com/docs/affordability/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_offers",
        publicName: "Offers & Discounts",
        description: "Create bank-level offers, cashbacks, and discounts applied at checkout.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 15,
        capabilityStatus: "NEW_INTEGRATION_REQUIRED",
        ...
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Offers API available. Not yet implemented in RasoKart. Offers are applied automatically at Checkout once created via API or Dashboard.",
        docsUrl: "https://razorpay.com/docs/offers/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_route",
        publicName: "Route (Marketplace Split)",
        description: "Split payments among multiple sub-merchants or accounts in real time.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 16,
        capabilityStatus: "RAZORPAY_APPROVAL_REQUIRED",
        officialApiAvailable: true,
        officialSdkAvailable: false,
        testModeStatus: "approval_required",
        liveModeStatus: "approval_required",
        webhookSupport: true,
        webhookEvents: ["transfer.processed", "transfer.failed", "reversal.created"],
        approvalRequired: true,
        approvalReason: "Razorpay Route activation required. Submit Marketplace onboarding documents to Razorpay.",
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Marketplace split-payment Route API. Requires separate activation. Not yet planned for RasoKart.",
        docsUrl: "https://razorpay.com/docs/route/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_checkout",
        publicName: "Standard Checkout (JS SDK)",
        description: "Embedded payment modal — the primary Razorpay customer-facing checkout experience.",
        isEnabled: true,
        status: "live",
        sortOrder: 17,
        capabilityStatus: "ALREADY_LIVE_IN_RASOKART",
        ...
        merchantAccess: false,
        customerFacingModule: true,
        implNotes: "Razorpay Checkout JS loaded from checkout.razorpay.com on the merchant deposits page. All payment methods (card, UPI, netbanking, wallet) handled by the modal.",
        docsUrl: "https://razorpay.com/docs/payment-gateway/web-integration/standard/",
      },
      {
        providerKey: "razorpayx",
        productKey: "razorpayx_payouts",
        publicName: "RazorpayX Payouts",
        description: "Programmatic bank transfers, UPI payouts, and wallet credits via RazorpayX.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 18,
        capabilityStatus: "PARTIALLY_IMPLEMENTED",
        ...
        liveModeStatus: "verification_required",
        webhookSupport: true,
        webhookEvents: ["payout.created", "payout.updated", "payout.pending", "payout.processed", "payout.reversed", "payout.rejected", "payout.failed"],
        approvalRequired: false,
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "RazorpayX activation probe implemented (verifyRazorpayXActivation). Actual payout disbursement flow not yet built. Requires separate RAZORPAY_X_KEY_ID + RAZORPAY_X_SECRET env vars.",
        docsUrl: "https://razorpay.com/docs/razorpayx/payouts/",
      },
      {
        providerKey: "razorpayx",
        productKey: "razorpayx_fund_accounts",
        publicName: "RazorpayX Fund Accounts",
        description: "Manage beneficiary fund accounts (bank, UPI, wallet) for payout workflows.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 19,
        capabilityStatus: "PARTIALLY_IMPLEMENTED",
        ...
        merchantAccess: false,
        customerFacingModule: false,
        implNotes: "Fund Accounts (beneficiaries) are prerequisites for RazorpayX Payouts. Activation probe in place; CRUD for fund accounts not yet implemented. Requires same RazorpayX activation as razorpayx_payouts.",
        docsUrl: "https://razorpay.com/docs/razorpayx/fund-accounts/",
      },
      {
        providerKey: "razorpay",
        productKey: "razorpay_razorpayme",
        publicName: "Razorpay.me (Profile Page)",
        description: "Public payment profile page at razorpay.me/<username> — no API required.",
        isEnabled: false,
        status: "coming_soon",
        sortOrder: 20,
        capabilityStatus: "NOT_SUPPORTED_BY_API",
        officialApiAvailable: false,
        officialSdkAvailable: false,
        testModeStatus: "dashboard_only",
        liveModeStatus: "dashboard_only",
        ...
        merchantAccess: true,
        customerFacingModule: true,
        implNotes: "Razorpay.me profile pages are managed exclusively through the Razorpay Dashboard. No REST API exists. Not applicable to RasoKart.",
        docsUrl: "https://razorpay.com/payment-buttons/razorpay-payment-link",
      },
    ];

    for (const product of RAZORPAY_CAPABILITY_PRODUCTS) {
      await db
        .insert(providerProductsTable)
        .values(product)
        .onConflictDoUpdate({
          target: providerProductsTable.productKey,
          set: {
            ...
            capabilityStatus: product.capabilityStatus,
            officialApiAvailable: product.officialApiAvailable,
            officialSdkAvailable: product.officialSdkAvailable,
            testModeStatus: product.testModeStatus,
            liveModeStatus: product.liveModeStatus,
            webhookSupport: product.webhookSupport,
            webhookEvents: product.webhookEvents,
            approvalRequired: product.approvalRequired,
            approvalReason: product.approvalReason,
            merchantAccess: product.merchantAccess,
            customerFacingModule: product.customerFacingModule,
            docsUrl: product.docsUrl,
          },
        });
    }
    logger.info({ count: RAZORPAY_CAPABILITY_PRODUCTS.length }, "razorpay_capability_products_seeded");
  } catch (capErr) {
    logger.warn({ err: capErr }, "razorpay_capability_seed_skipped — schema guard may not have run yet");
  }
