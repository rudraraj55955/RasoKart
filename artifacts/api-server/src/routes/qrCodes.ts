...
router.get("/", async (req, res) => {
  ...
  if (search) conditions.push(or(ilike(qrCodesTable.label, `%${search}%`), ilike(qrCodesTable.payload, `%${search}%`))!);
  ...
  const rows = await db.select({ qr: qrCodesTable, merchantName: merchantsTable.businessName })
    ...
    .orderBy(sql`${qrCodesTable.createdAt} DESC`);

  res.json({
    data: rows.map(r => ({ ...r.qr, merchantName: r.merchantName ?? null })),
...
router.post("/", async (req, res) => {
  ...
  const { type, label, payload, amount } = req.body;
  if (!type || !payload) { res.status(400).json({ error: "type and payload required" }); return; }
  const [row] = await db.insert(qrCodesTable).values({ merchantId, type, label: label ?? null, payload, amount: amount ?? null }).returning();
