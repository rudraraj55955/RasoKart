...
describe("buildFailoverEventList — both chain_exhausted and threshold_alert", () => {
  ...
  it("two distinct chain_exhausted events (different merchants) are NOT collapsed", () => {
    ...
    assert.equal(events.length, 2, "Different merchant events must not be collapsed");
  });

  it("events are sorted newest-first", () => {
    const older = new Date("2026-08-01T06:00:00.000Z");
    const newer = new Date("2026-08-01T08:00:00.000Z");

    const events = buildFailoverEventList({
      chainRows: [{ id: 1, metadata: { triggerMerchantId: 1, exhaustedAt: older.toISOString() }, createdAt: older }],
      thresholdRows: [{ id: 2, metadata: { failureCount: 5, triggerMerchantId: 2, outageStartedAt: newer.toISOString() }, createdAt: newer }],
      recoveryRows: [],
    });

    assert.equal(events.length, 2);
    assert.ok(
      new Date(events[0]!.createdAt) >= new Date(events[1]!.createdAt),
...
// =============================================================================
// acknowledge endpoint — snooze persistence (unit coverage via helper mock)
// =============================================================================
//
// The POST /api/smart-routing/failover-events/acknowledge endpoint writes
// FAILOVER_ALERT_SNOOZED_UNTIL to system_config. We cover the snooze-suppression
// behaviour through the maybeFireFailoverAlert snooze tests above. Here we
// directly verify the one-shot pure-data invariant: buildFailoverEventList
// returns all events unchanged regardless of snooze state, because snooze
// is only applied by maybeFireFailoverAlert (not by the event list itself).

describe("buildFailoverEventList — snooze state does NOT affect event list shape", () => {
  it("returns the same events whether or not a snoozedUntil key is present in recovery rows", () => {
    // Use a fixed base time and anchor `now` to 1 hour after the event so it
    // stays well within the 2-hour stale threshold regardless of wall-clock time.
    const t = new Date("2026-08-03T10:00:00.000Z");
    const nowAnchor = t.getTime() + 60 * 60 * 1000; // 1 hour later — event is "recent"

    // buildFailoverEventList is a pure function; it only processes
    // gateway_failover_exhausted / gateway_chain_exhausted / gateway_recovered
    // notification rows. FAILOVER_ALERT_SNOOZED_UNTIL is a system_config key
    // read by the route, not passed to this helper. So the helper must return
    // the same event list whether or not a snooze is active.
    const eventsWithoutSnooze = buildFailoverEventList({
      chainRows: [{ id: 1, metadata: { triggerMerchantId: 5, exhaustedAt: t.toISOString() }, createdAt: t }],
      thresholdRows: [],
      recoveryRows: [],
      now: nowAnchor,
    });

    // Pretend we also have a gateway_recovered row (unrelated to SNOOZED_UNTIL):
    const recoveredAt = new Date(t.getTime() + 3600_000).toISOString();
    const eventsWithRecovery = buildFailoverEventList({
      chainRows: [{ id: 1, metadata: { triggerMerchantId: 5, exhaustedAt: t.toISOString() }, createdAt: t }],
      thresholdRows: [],
      recoveryRows: [{ metadata: { outageStartedAt: t.toISOString(), recoveredAt, durationSeconds: 3600 }, createdAt: new Date(recoveredAt) }],
      now: nowAnchor,
    });

    // Event count is the same; only status differs (resolved vs ongoing)
    assert.equal(eventsWithoutSnooze.length, 1);
    assert.equal(eventsWithRecovery.length, 1);
    assert.equal(eventsWithoutSnooze[0]!.id, eventsWithRecovery[0]!.id, "Same event id regardless of recovery");
    assert.equal(eventsWithoutSnooze[0]!.status, "ongoing", "No recovery + recent event → ongoing");
    assert.equal(eventsWithRecovery[0]!.status, "resolved", "With matching recovery row → resolved");
  });
});
