...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {stats?.demoDataOnly && (
        <div className="flex items-start gap-3 rounded-lg border border-amber-500/30 bg-amber-500/8 px-4 py-3 text-sm text-amber-400">
          <FlaskConical className="w-4 h-4 shrink-0 mt-0.5" />
          <div className="flex-1 min-w-0">
            <p className="font-semibold">Demo environment — all data is seed data</p>
            <p className="text-xs text-amber-400/70 mt-0.5">
              All merchants, transactions, and wallet balances shown below belong to seeded demo accounts.
              KPIs will reflect real data once the first production merchant is onboarded.
            </p>
          </div>
        </div>
      )}
