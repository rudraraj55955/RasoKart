          ...
          <TableCell colSpan={6} className="bg-muted/20 pb-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-2 pt-2">
              ...
              <div>
                ...
                <pre className="text-xs bg-background/50 rounded p-3 overflow-x-auto border border-border/50 whitespace-pre-wrap">{log.responseBody ? JSON.stringify(JSON.parse(log.responseBody), null, 2) : "—"}</pre>
              </div>
            </div>
          </TableCell>
        </TableRow>
...
export default function AdminCallbacks() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-bold tracking-tight">Callback Logs</h1><p className="text-muted-foreground mt-1">Webhook delivery history</p></div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-10">No callback logs found</TableCell></TableRow>
