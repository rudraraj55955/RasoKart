...
**Backfilling legacy null rows:** if it's worth showing attribution immediately instead of waiting for the next real admin save, backfill `updatedByEmail` to a clearly-synthetic label (e.g. `"system (legacy config)"`, never a real admin's name) for rows where it's still null, gated with `WHERE updatedByEmail IS NULL` so it only fires once and never overwrites a real save. Preserve the original `updatedAt` by explicitly setting it to itself (`sql`${table.updatedAt}`` in Drizzle) — otherwise a column with `$onUpdate(() => new Date())` bumps the timestamp to "now" on the backfill UPDATE, making a years-old row look like it just changed.
